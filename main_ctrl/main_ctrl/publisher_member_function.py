...
GPIO.setup(trig_L, GPIO.OUT)                   # Trig=17 초음파 신호 전송핀 번호 지정 및 출력지정
GPIO.setup(echo_L, GPIO.IN)                    # Echo=18 초음파 수신하는 수신 핀 번호 지정 및 입력지정
GPIO.setup(trig_R, GPIO.OUT)                   # Trig=17 초음파 신호 전송핀 번호 지정 및 출력지정
GPIO.setup(echo_R, GPIO.IN)                    # Echo=18 초음파 수신하는 수신 핀 번호 지정 및 입력지정
GPIO.setup(trig_front, GPIO.OUT)                   # Trig=17 초음파 신호 전송핀 번호 지정 및 출력지정
...
class MinimalPublisher(Node):
    ...
    def timer_callback(self):
        try:
            ...
            imu_data.angular_velocity_covariance[0] = sonar(trig_L , echo_L) # LF sonar distance
            imu_data.angular_velocity_covariance[4] = sonar(trig_R , echo_R) # RF sonar distance
            imu_data.angular_velocity_covariance[8] = sonar(trig_front, echo_front)
            ...
            imu_data.linear_acceleration_covariance[1] =  Gz  #yaw
            ...
            self.euler = comp_filter(alpha=0.96, acc=Acc, gyro=Gyro, euler=self.euler,dt =dt)
            ...
            print(imu_data.angular_velocity_covariance[8])
            print(self.euler)
            self.publisher_.publish(imu_data)
            #
            self.i += 1
        except:
            self.get_logger().error("talker error!!!")
